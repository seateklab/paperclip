...
describe("runChildProcess", () => {
  ...
  it("preserves multibyte stdout across chunk boundaries", async () => {
    const expected = String.fromCodePoint(0x1f60a);
    ...
    const result = await runChildProcess(
      ...
      [
        ...
        [
          ...
          "process.stdout.write(Buffer.from([0x98, 0x8A]));",
        ].join(" "),
      ],
      {
        cwd: process.cwd(),
    ...
    expect(observed.join("")).toBe(expected);
  });
...
describe("renderPaperclipWakePrompt", () => {
  it("keeps the default local-agent prompt action-oriented", () => {
    ...
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("Always follow the language of the issue title, description, and latest user-facing comments");
