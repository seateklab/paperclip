...
describe.sequential("agent skill routes", () => {
  ...
  it("materializes the bundled CEO instruction set for default CEO agents", async () => {
    ...
    const bundle = mockAgentInstructionsService.materializeManagedBundle.mock.calls[0]?.[1] as
      | Record<string, string>
      | undefined;
    expect(bundle?.["AGENTS.md"]).toContain(
      "Always respond in the same language as the issue title, description, and latest user-facing comments",
    );
    expect(bundle?.["HEARTBEAT.md"]).toContain("Follow the issue language exactly");
  });
