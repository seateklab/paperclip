...
| `PAPERCLIP_BOOTSTRAP_ADMIN_EMAIL` | (unset) | Optional email allowlist for first-admin claim/bootstrap acceptance in `authenticated` mode; when set, only that signed-in email can claim the initial instance admin role |
