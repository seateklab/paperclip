...
On Windows, if embedded PostgreSQL fails because the default Paperclip home is not writable or a permission error is raised during startup, set `PAPERCLIP_HOME` to a writable path before retrying:
